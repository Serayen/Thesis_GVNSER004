...
dat <- data.Passerina.C %>% ungroup(by = "Site") %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
...
#Thesium data
dat.th <- data.Thesium.C %>% ungroup(by = "Site") %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
dat.th <- dat.th  %>% ungroup(by = "CT") %>% mutate(CT = ifelse(CT == "Treatment",1,0))
...
cat("
    ...
    beta[3] ~ dnorm(0, 0.001) #B Climate1
    ...
    mu.lam[i,t] = beta[1] + beta[2]*z1[i] + beta[3]*z2[i,t] + eps.plot[i]
...
inits <- function()
{
  list(sig.proc = runif(1, 0, 5),sig.plot=runif(1, 0, 5), beta =runif(3,0,1),  sig.obs = runif(1, 0, 10), N=  matrix(c(runif(dim(Y)[1],0,5000),rep(NA,(dim(Y)[2]-1+(for_per))*dim(Y)[1])), nrow=dim(Y)[1]))
} 
...
ssm2 <- jags(jags.data, inits, parameters, "Passerina_Thesium_R.jag", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb) 

ssm2 <- autojags(ssm2, n.iter=1000)

setwd("C:/Users/Serayen/Research Work/Data")
#save(ssm2, file = "Passerina_ThR_SSM.RData")
#load("Passerina_ThR_SSM.RData")
setwd("C:/Users/Serayen/Research Work/Code")
...
cat("
    ...
    beta[2] ~ dnorm(0, 0.01) #B Treatmment
...
#Start Gibbs sampler: Run model in JAGS and save results in object called ssm
#***Useful Data is saved in objects so it is not necessary to run this to check the rest of the code
ssm3 <- jags(jags.data, inits, parameters, "ElytropappusF.jag", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb) 

#ssm3 <- autojags(ssm3, n.iter=1000)

setwd("C:/Users/Serayen/Research Work/Data")
#save(ssm3, file = "ElytropappusF_SSM.RData")
#load("ElytropappusF_SSM.RData")
setwd("C:/Users/Serayen/Research Work/Code")
