...
#Reseeders
load("Selago_Plot_C.RData")
load("Aspalathus_Plot_C.RData")
#Resprouters
load("Agathosma_Plot_C.RData")
load("Wahlenbergia_Plot_C.RData")
setwd("C:/Users/Seray/Research Work/Code")
#Set Fynbos to 1 and Renostoveld as 0
dat <- data.Selago.C  %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
...
#Aspalathus data
dat.asp <- data.Aspalathus.C %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
dat.asp <- dat.asp  %>% mutate(CT = ifelse(CT == "Treatment",1,0))
dat.asp_ren <- dat.asp %>% filter(Site == 0)
dat.asp_fyn <- dat.asp %>% filter(Site == 1)
dat.asp_fyn <-dat.asp_fyn %>% add_column(seq_plotNo = seq(1,12))

#Agathosma data
dat.aga <- data.Agathosma.C %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
dat.aga <- dat.aga  %>% mutate(CT = ifelse(CT == "Treatment",1,0))
dat.aga_ren <- dat.aga %>% filter(Site == 0)
dat.aga_fyn <- dat.aga %>% filter(Site == 1)
dat.aga_fyn <-dat.aga_fyn %>% add_column(seq_plotNo = seq(1,12))

#Aspalathus data
dat.wal <- data.Wahlenbergia.C %>% mutate(Site = ifelse(Site == "Fynbos",1,0))
dat.wal <- dat.wal  %>% mutate(CT = ifelse(CT == "Treatment",1,0))
dat.wal_ren <- dat.wal %>% filter(Site == 0)
dat.wal_fyn <- dat.wal %>% filter(Site == 1)
dat.wal_fyn <-dat.wal_fyn[1:12,] %>% add_column(seq_plotNo = seq(1,12))
...
y2 = as.matrix(dat.asp[, c(8:13)])
y3 = as.matrix(dat.aga[, c(8:13)])
y4 = as.matrix(dat.wal[, c(8:13)])
...
ssm3 <- jags(jags.data, inits, parameters, "MV_Microdon.jag", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb) 

ssm3 <- autojags(ssm3, n.iter=1000)
...
ssm3$BUGSoutput$mean$beta

NT.out <- ssm3$BUGSoutput$mean$N
N.out <- NT.out[,,1]
...
#Asp plots
#Treat
Plot_ren_T(N_sum2, NT.out[,,2], dat.asp, 1 )
#control
Plot_ren_T(N_sum2, NT.out[,,2], dat.asp, 0 )

#Aga plots
#Treat
Plot_ren_T(N_sum3, NT.out[,,3], dat.aga, 1 )
#control
Plot_ren_T(N_sum3, NT.out[,,3], dat.aga, 0 )

#Wal plots
#Treat
Plot_ren_T(N_sum4, NT.out[,,4], dat.wal, 1 )
#control
Plot_ren_T(N_sum4, NT.out[,,4], dat.wal, 0 )
...
library(xtable)
xtable(ssm3$BUGSoutput$mean$beta)
