...
cat("
    ...
    N[i,1] ~ dunif(0, 15000000)
    
    }
    #Priors
    sig.proc ~ dunif(0,1000000)
    sig2.proc <- pow(sig.proc,2)
    tau.proc <- pow(sig.proc, -2)
    
    sig.obs ~ dunif(0,1000000)
    sig2.obs <- pow(sig.obs, 2)
    tau.obs <- pow(sig.obs, -2)
    
    
    sig.plot ~ dunif(0,1000000)
    sig2.plot <- pow(sig.plot , 2)
    tau.plot <- pow(sig2.plot, -2)
    
    beta[1] ~ dnorm(0, 0.00001) #intercept
    beta[2] ~ dnorm(0, 0.00001) #B Treatmment
    ...
    mu.lam[i,t] = beta[1] + beta[2]*z1[i] + beta[3]*z2[t] + beta[4]*z3[t] + beta[5]*z4[t] + eps.plot[i]
...
Y = as.matrix(dat[, c(6:11)])  #Observed volume of population

#site = as.matrix(dat[,2]) #Site and treatment as categorical varibles
treat = as.matrix(dat[,5])
...
#Setup JAGS data
jags.data <- list(y = Y, T=6, x = dim(Y)[1], fp = for_per, z1 = as.numeric(treat),  z2 = AirTC, z3 = RH, z4 = WS )

# Function to generate starting values
inits <- function()
{
  list(sig.proc = runif(1, 0, 5),sig.plot=runif(1, 0, 5), beta =runif(5,0,1),  sig.obs = runif(1, 0, 10), N=  matrix(c(runif(dim(Y)[1],0,5000),rep(NA,(dim(Y)[2]-1+(for_per))*dim(Y)[1])), nrow=dim(Y)[1]))
} 
...
ssm <- jags(jags.data, inits, parameters, "Microdon.jag", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb) 

ssm <- autojags(ssm, n.iter=1000)

N.out <- ssm$BUGSoutput$mean$N
...
preds <- as_tibble(cbind(dat[,1:5],tmp))
preds_T <- preds %>% filter(CT == 1)
preds_C <- preds %>% filter(CT == 0) 

N.out_T <- as.matrix(preds_T[, c(6:(11+for_per))])
N.out_C <- as.matrix(preds_C[, c(6:(11+for_per))])

Y_T = as.matrix(dat_T[, c(6:11)])
Y_C = as.matrix(dat_C[, c(6:11)])

#Summary stats of estimations population volumes
N.sum <- ssm$BUGSoutput$summary
...
period <- (6+for_per)
N.sum <- ssm$BUGSoutput$summary
No_entries = (2*12*(period-1)) + 12*period +1
deviance.sum <- N.sum[((12*period)+1),]
N.sum<- rbind(N.sum[1:((12*period)),], N.sum[((12*period)+2):No_entries,])
No_entries = (2*12*(period-1)) + 12*period 
